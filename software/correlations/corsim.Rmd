...
The idea with detecting correlated gene pairs is to mitigate dependence on explicit subpopulation identification for follow-up studies.
...
Tie breaking ensures that the correlation is not purely driven by the few cells that happen to express both genes,
    thus favouring genes with correlations driven by expression across many cells in the data set.
...
# With multiple rounds of tie breaking:
set.seed(1000)
correlatePairs(rbind(g1, g2), null.iter=1e4)
```

Of course, if there are many ties, the final correlation and p-value may be heavily dependent on the manner of tie breaking.
