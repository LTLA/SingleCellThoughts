...
set.seed(200)
detect.tab <- "detect_trajectory.txt"
above.tab <- "above_trajectory.txt"
new.file <- TRUE
ngenes <- 1000
ncells <- 500
nspikes <- 50
...
for (grad in c(-4, -3, -2)) {
    ...
    write.table(data.frame(Grad=grad, rbind(colMeans(do.call(rbind, collected.d)))),
                file=detect.tab, append=!new.file, col.names=new.file, row.names=FALSE, sep="\t", quote=FALSE)
    write.table(data.frame(Grad=grad, rbind(colMeans(do.call(rbind, collected.a)))),
