...
Bootstrapping aims to determine the stability of the clusters in a hypothetical replicate case.
...
and bootstrapping only provides the former.

Another problem is that bootstrapping fails to capture the true variability across samples.
A bootstrap replicate is not a good representative of a real replicated experiment,
which is generated from a different set of cells with different cell type composition and probably changes in expression.
This means that bootstrapping will be overly optimistic with respect to what it considers to be reproducible.

On the flipside, we might actually consider the comparisons between clusters to be far too strict.
The clustering algorithm may collapse or split apart clusters due to the noise introduced by bootstrapping, 
...
In such cases, the scientific conclusions are reproducible across bootstrap replicates even if the exact co-assignments are not,
and bootstrap-based stability calculations would be overly pessimistic.
