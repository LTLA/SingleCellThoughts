...
Fortunately, this extreme change in p-values is probably uncommon, the separation between cell types on the CDR should be minimal.
Even if the population means are well-separated, there should be enough noise that the two distributions are quite well mixed.
This reduces the confounding impact, as `cov` represents a much noisier proxy for `g` that results in a poorer fit.
The noise is further exacerbated when `cov` is scaled in the linear model to match the magnitude of difference due to `g`.
...
This gets a lot better if you throw in the CDR as a covariate.
