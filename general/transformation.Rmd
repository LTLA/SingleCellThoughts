...
The `r Githubpkg("ChristophH/sctransform")` package fits a negative binomial GLM to each gene and yields Pearson residuals as the transformed values [@hafemeister2019normalization].
This is not dissimilar to the `zscoreNBinom` function from `r Biocpkg("edgeR")`, which does a similar conversion of count data to a continuous space based on the NB distribution.
Users can then operate on the residuals in downstream exploratory analyses like dimensionality reduction and clustering.

One problem is that the function does not actually achieve variance stabilization, even in a simple case.
There are several major issues in the example below -
 the irregular behaviour at low abundances,
 the lack of any increase in the variance for the subset of "highly variable genes",
 and the complete loss of all variation at high abundances.
...
vst.out <- vst(counts, show_progress=FALSE)
stuff <- get_residuals(vst.out, counts,
    cell_attr=data.frame(log_umi=log(colSums(counts))),
    show_progress=FALSE)

par(mfrow=c(1,2))
plot(means, matrixStats::rowVars(stuff), main="scTransform")
plot(means, matrixStats::rowVars(counts), main="Untransformed")
```

The use of the log-total count as a covariate in the GLM fit is also concerning.
Any biology correlated with the total count (e.g., involving differences in total RNA content) may be regressed out.
Below is an example of this effect using pre-processed `r Biocpkg("simpleSingleCell", "tenx.html", "PBMC data")`,
where `sctransform`ed data results in loss of marker expression in over half of the platelet population.
(There is also some gain of expression in non-expressing cells, which is not expected for a transformation method.)

```{r, fig.wide=TRUE, eval=file.exists("pbmc_data.rds")}
# To construct pbmc_data.rds:
#   git clone https://github.com/MarioniLab/simpleSingleCell
#   cd simpleSingleCell/vignettes
#   R --slave --no-save -e "rmarkdown::render('tenx.Rmd')"
library(SingleCellExperiment)
sce <- readRDS("pbmc_data.rds")

# AL: as of time of writing, cluster 9 was the platelet cluster;
# see the link to the PBMC data above for updates. Any other 
# cluster other than '9' can be used as the counterpart.
platelets <- "9"
sce <- sce[,sce$Cluster %in% c(platelets,"2")]

vst.out <- vst(counts(sce), show_progress=FALSE)
stuff <- get_residuals(vst.out, counts(sce),
    cell_attr=data.frame(log_umi=log(colSums(counts(sce)))),
    show_progress=FALSE)

par(mfrow=c(1,3))
for (gene in c("PF4", "SDPR", "PPBP")) {
    plot(y=stuff[gene,], x=logcounts(sce)[gene,], xlab="Log-counts",
        ylab="scTransformed", main=gene, pch=16,
        col=ifelse(sce$Cluster==platelets, "blue", "red"))
}
```

Of course, platelets are a rather extreme example, but one can imagine more subtle cases where real biology is lost.
The code demonstrates this effect more clearly, where most library size-associated variation is discarded.
This occurs even for those genes where expression is negatively correlated with library size, 
such that any trend is unlikely to be a technical ecffect.
...
counts[1,] <- rnbinom(ncells, mu=1:500, size=10)
counts[2,] <- rnbinom(ncells, mu=500:1, size=10)
rownames(counts) <- sprintf("GENE_%i", seq_len(ngenes))
colnames(counts) <- sprintf("CELL_%i", seq_len(ncells))
...
var(stuff[1,])
var(stuff[2,])
summary(matrixStats::rowVars(stuff))
```

The transformed values are also uninterpretable, with no relation to the original scale of the (log-)counts at all.
This is not a problem for exploratory analyses but makes them useless for meaningful differential expression analyses.
Thus, it is rather unfortunate that they are being recommended for such a purpose.
