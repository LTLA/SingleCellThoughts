...
One problem, as mentioned before, is that it is impossible to achieve variance stabilization at low counts.
...
plot(means, matrixStats::rowVars(vst.out$y), log="xy")
```

While the use of the log-total count as a covariate in the GLM fit is somewhat concerning, 
it is protected by using the regularized (i.e., abundance-trended) estimates of all coefficients.
...
vst.out <- vst(counts, show_progress=FALSE)
smoothScatter(log(counts+1), vst.out$y)
```
